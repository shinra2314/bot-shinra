...
const { COLORS, errorPanel, panel, reply, successPanel, update } = require('../ui/components');
    ...
    async execute(interaction, context) {
      ...
      return reply(interaction, panel({
        ...
        lines: history.length
          ? history.map((item) => `**${item.type}** — ${formatDateTime(item.createdAt)}\nМодератор: ${mentionUser(item.moderatorId)}\n${item.reason || 'без причины'}`)
          : ['История пустая.']
      }), { ephemeral: true });
    }
  },
