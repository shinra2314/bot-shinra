...
const HELP_CATEGORIES = [
  {
    ...
    commands: [
      ...
      ['`/панель`', 'опубликовать интерактивную панель семейства (админ)'],
  ...
  },
  {
    ...
    commands: [
      ...
      ['`/casino слоты`', 'слоты на монеты'],
      ['`/casino рулетка`', 'рулетка 0-36'],
      ['`/casino лоты`', 'моментальные лотерейные лоты']
    ]
  },
  {
    id: 'roles',
  ...
  },
  {
    id: 'fun',
    label: 'Развлечения',
    description: 'Монетка, дуэли, отношения, реакции',
    commands: [
      ['`/coinflip`', 'подбросить монетку'],
      ['`/duel`', 'бросить вызов на монеты'],
      ['`/casino кости`', 'казино-кости'],
      ['`/casino статистика`', 'казино-профиль'],
      ['`/love профиль`', 'любовный профиль пары'],
      ['`/love предложить`', 'предложить отношения'],
      ['`/love действие`', 'повысить настроение пары'],
      ['`/love подарок`', 'подарить предмет паре'],
...
function helpComponents(categoryId = 'main') {
  ...
  return panel({
    eyebrow: `Помощь • раздел ${HELP_CATEGORIES.findIndex((item) => item.id === category.id) + 1}/${HELP_CATEGORIES.length}`,
    icon: HELP_ICONS[category.id] || ICONS.info,
    ...
    async execute(interaction, context) {
      ...
      return reply(
        ...
        panel({
          ...
          icon: ICONS.success,
          eyebrow: 'Модерация Onix',
          description: `${mentionUser(interaction.user.id)}, жалоба на ${mentionUser(target.id)} принята.\n**Ожидайте ответа от модерации сервера.**`,
    ...
    async execute(interaction, context) {
      ...
      return reply(
        ...
        panel({
          ...
          icon: ICONS.voice,
          eyebrow: 'Активность Onix',
          description: `${mentionUser(target.id)}`,
          thumbnail: target.displayAvatarURL({ size: 256 }),
          color: COLORS.info,
          stats: [
            { icon: ICONS.voice, name: 'Сейчас', value: member?.voice?.channelId ? 'в голосовом канале' : 'не в войсе' },
            { icon: ICONS.time, name: 'За сутки', value: formatMinutes(dayMinutes + liveMinutes) },
            { icon: '🕓', name: 'За всё время', value: formatMinutes(totalMinutes) },
            { icon: ICONS.tops, name: 'Место в топе', value: rank }
          ],
          statColumns: 2
        })
      );
