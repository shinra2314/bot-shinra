...
const commands = [
  {
    ...
    async execute(interaction, context) {
      ...
      if (subcommand === 'статистика') {
        ...
        return reply(
          ...
          panel({
            ...
            color: COLORS.games,
            fields: [
              { name: 'Очки', value: String(profile.eventPoints || 0) },
              { name: 'Победы', value: String(profile.eventWins || 0) }
            ]
          })
        );
      }
      ...
      if (subcommand === 'участвовать') {
        ...
        if (!profile.achievements.includes('Участник ивента')) profile.achievements.unshift('Участник ивента');
      ...
      for (const userId of event.participants) {
        ...
        profile.achievements.unshift(`Награда за ивент: ${event.name}`);
      ...
      if (subcommand === 'статистика') {
        ...
        return reply(
          ...
          panel({
            ...
            color: COLORS.games,
            fields: [
              { name: 'Игры', value: String(profile.mafia.games || 0) },
              { name: 'Победы', value: `${profile.mafia.wins || 0} (${percent(profile.mafia.wins, profile.mafia.games)})` },
              { name: 'Рейтинг', value: String(profile.mafia.rating || 1000) },
              { name: 'MVP', value: String(profile.mafia.mvp || 0) }
            ]
          })
        );
      }
    ...
    async execute(interaction, context) {
      ...
      return reply(
        ...
        panel({
          ...
          color: COLORS.games,
          fields: [
            { name: 'Игры', value: String(profile.closes.games || 0) },
            { name: 'Победы', value: `${profile.closes.wins || 0} (${percent(profile.closes.wins, profile.closes.games)})` },
            { name: 'Рейтинг', value: String(profile.closes.rating || 0) }
          ]
        })
      );
    }
